...
var file = require('../models/readFile.js')
...
router.post('/:zoom/:lat/:lng', function(req, res, next) {
	...
	if(type == 'initial'){
		...
		result = greedy.SOSspatial(bounds,k,Distance);
		var start = Date.now();
		var sim_score = getGreedyScore(result.selected,result.proList,bounds);
		var end = Date.now();
	...
	if(type == 'Random'){
		...
		var candidate = file.getData_lat_lon(bounds);
		var start = Date.now();
  ...
  if (err) console.error(err);
	...
	if(type == 'MaxMin'){
		var candidateAll = file.getData_lat_lon(bounds);
		var length = Math.round(candidateAll.length/2);
